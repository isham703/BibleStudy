        ...
        GeometryReader { geometry in
            ZStack {
                ...
                VStack(spacing: 0) {
                    ...
                    Spacer()
                        .frame(height: HomeShowcaseTheme.Spacing.xl)
    ...
    private var headerSection: some View {
        VStack(spacing: 4) {
            Text("SACRED THRESHOLD")
                .font(HomeShowcaseTypography.Threshold.headerTitle)
                .tracking(6)
            ...
            Text("Choose your path")
                .font(HomeShowcaseTypography.Threshold.headerSubtitle)
            ...
            if let prevRoom = currentRoom.previous {
                HStack(spacing: 4) {
                    ...
                    Text(prevRoom.shortName)
                        .font(HomeShowcaseTypography.Threshold.swipeHint)
                }
        ...
        NavigationLink(destination: room.aiFeature.destinationView) {
            VStack(spacing: HomeShowcaseTheme.Spacing.xl) {
                ...
                Text(room.name.uppercased())
                    .font(HomeShowcaseTypography.Threshold.roomName)
                    .tracking(-1)
                ...
                Text(room.description)
                    .font(HomeShowcaseTypography.Threshold.roomDescription)
                ...
                if let progress = room.progress {
                    ...
                    if room == .memoryPalace {
                        VStack(spacing: 8) {
                            ...
                            Text(progress)
                                .font(HomeShowcaseTypography.Threshold.progressText)
                                .foregroundStyle(Color.white.opacity(0.5))
                        }
                    } else {
                        Text(progress)
                ...
                Spacer()
                    .frame(height: HomeShowcaseTheme.Spacing.md)

                // CTA Button
                HStack(spacing: 8) {
                    Text("Enter \(room.shortName)")
                        .font(HomeShowcaseTypography.Threshold.ctaButton)
