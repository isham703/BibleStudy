...
extension View {
    ...
    func glassCard() -> some View {
        self
            ...
            .clipShape(RoundedRectangle(cornerRadius: HomeShowcaseTheme.CornerRadius.card))
            .overlay(
...
enum HomeShowcaseHaptics {
    ...
    // MARK: - Memory Palace Haptics

    // Candlelit Palace (Warm & Organic)
    static func palacePress() {
        UIImpactFeedbackGenerator(style: .soft).impactOccurred(intensity: 0.4)
    }

    static func palaceWordReveal() {
        UIImpactFeedbackGenerator(style: .light).impactOccurred(intensity: 0.4)
    }

    static func palacePhaseComplete() {
        // Double-tap effect
        UIImpactFeedbackGenerator(style: .light).impactOccurred()
    ...
    static func palaceRoomComplete() {
        UIImpactFeedbackGenerator(style: .medium).impactOccurred()
    ...
    // Scholarly Study (Precise & Tactile)
    static func studyPress() {
        UIImpactFeedbackGenerator(style: .rigid).impactOccurred(intensity: 0.3)
    }

    static func studyWordReveal() {
        UIImpactFeedbackGenerator(style: .light).impactOccurred()
    }

    static func studyPhaseComplete() {
        UIImpactFeedbackGenerator(style: .light).impactOccurred()
    }

    static func studyRoomComplete() {
        UIImpactFeedbackGenerator(style: .medium).impactOccurred()
    }

    // Celestial Cathedral (Vast & Reverberant)
    static func celestialPress() {
        UIImpactFeedbackGenerator(style: .soft).impactOccurred(intensity: 0.6)
    }

    static func celestialWordReveal() {
        UIImpactFeedbackGenerator(style: .soft).impactOccurred(intensity: 0.7)
    }

    static func celestialPhaseComplete() {
        UIImpactFeedbackGenerator(style: .heavy).impactOccurred(intensity: 0.4)
    }

    static func celestialRoomComplete() {
        // Triple cascade
        UIImpactFeedbackGenerator(style: .light).impactOccurred()
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.15) {
            UIImpactFeedbackGenerator(style: .medium).impactOccurred()
