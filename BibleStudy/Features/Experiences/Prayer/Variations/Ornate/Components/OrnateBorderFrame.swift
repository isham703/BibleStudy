...
struct OrnateBorderFrame: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            ZStack {
                    ...
                    .stroke(
                        LinearGradient(
                            colors: [
                                ...
                                DeepPrayerColors.roseAccent.opacity(0.2),
                                DeepPrayerColors.goldAccent.opacity(0.3)
                            ],
                ...
                RoundedRectangle(cornerRadius: cornerRadius)
                    .stroke(
                        DeepPrayerColors.goldAccent.opacity(0.1),
                        lineWidth: 3
                    )
                    .blur(radius: 4)
                    .padding(inset)
    ...
    private var cornerOrnament: some View {
        ZStack {
            // Outer circle
            Circle()
                .stroke(DeepPrayerColors.goldAccent.opacity(0.4), lineWidth: 1)
                .frame(width: 12, height: 12)

            // Inner dot
            Circle()
                .fill(DeepPrayerColors.goldAccent.opacity(0.6))
